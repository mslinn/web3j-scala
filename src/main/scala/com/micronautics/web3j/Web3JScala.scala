...
object Web3JScala {
  ...
  /** Invokes fn on all elements observed from the given Flowable[T] */
  def observe[T](observable: Flowable[T])
                (fn: T => Unit): Unit =
    observable.subscribe(fn(_))

  /** Only runs fn on the first n elements observed from the given Flowable[T] */
  def observe[T](n: Long)
  ...
  def wrapAbi(
    ...
    "bin/web3j", "solidity",
    "generate", "-b", s"$inputDirectory/${toCamelCase(filename)}.bin", "-a", s"$inputDirectory/${toCamelCase(filename)}.abi",
    "-o", outputDirectory,
    "-p", packageName
  )
